...
debug('chutney', 'creating WebSocket reporter')
const reporter = websocket('wss://' + location.host + '/')
tap.pipe(reporter)

reporter.on('connect', () => {
	debug('chutney', 'WebSocket reporter connected')
})
onFinish(tap, () => setTimeout(() => {
	debug('chutney', 'closing WebSocket reporter')
	reporter.end()
	reporter.destroy()
	logger.release()
}, 100))

debug('chutney', 'setting up visual log')
...
reporter.once('connect', () => {
	debug('chutney', 'attaching test script')
